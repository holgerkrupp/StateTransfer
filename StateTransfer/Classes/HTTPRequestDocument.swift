...
struct HTTPRequestDocument: FileDocument {
    static var readableContentTypes: [UTType] { [.json, .propertyList, UTType(filenameExtension: "request")!] }
        ...
        do {
            ...
            if let plistObject = try PropertyListSerialization.propertyList(from: plistData, options: [], format: nil) as? [String: Any] {
                ...
                let jsonDict: [String: Any] = [
                    "header": (plistObject["headers"] as? [[String: Any]])?.map { header in
                        [
                    ...
                    "body": (plistObject["usingHTTPBody"] as? Bool == true) ? "BODY TEXT" : "",
                    "follorRedirects": plistObject["followRedirect"] as? Bool ?? true,
                    "parameterEncoding": "Form encoded",
                    "method": (plistObject["httpMethod"] as? String ?? "GET").lowercased(),
                    "parameters": (plistObject["parameters"] as? [[String: Any]])?.map { param in
                        [
                            "id": UUID().uuidString,
